...
export const metadata: Metadata = {
  title: 'Nuestros Valores | Multi Impresiones AH',
  description: 'Conoce los valores fundamentales que guían nuestro trabajo en Multi Impresiones AH.',
};

async function getValuesContent() {
  const { data } = await supabase
    .from('store_settings')
    .select('key, value')
    .in('key', ['values_title', 'values_content']);
  ...
  return {
    title: settings.values_title || 'Nuestros Valores',
    content: settings.values_content || 'Nuestros valores fundamentales aparecerán aquí.',
  };
}

export default async function NuestrosValoresPage() {
  const { title, content } = await getValuesContent();
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pt-8 pb-20">
      <div className="max-w-4xl mx-auto px-4 w-full">
        ...
        <nav className="mb-8 flex items-center text-sm text-gray-500">
          ...
          <span className="text-azul-profundo font-medium">Nuestros Valores</span>
        </nav>
